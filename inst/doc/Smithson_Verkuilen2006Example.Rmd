---
title: "cdfquantreg: Smithson & Verkuilen data example"
author: "Yiyun Shou, Michael Smithson"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Juror & Stree data example}
...
* `crc99`: The ratings of confidence levels with rescaling into the (0, 1) interval to avoide 1 and 0 values.
* `vert`: was the dummy variable for coding the conditions of verdict types, whereas 
    + `verdict = -1` indcates responses from the two-option verdict condition and 
    + `verdict = 1` indcates responses from the three-option verdict condition.
* `confl`: was the dummy variable for coding the conflict conditions, whereas 
    + `confl = -1` indcates the no-conflict condition and 
    + `confl = 1` indcates the conflict condition.
...
#### Fit predicted values with responses in each condition
...
Currently three types of residuals are avaiable: raw residuals, Pearson's residuals and deviance residuals.
...
```{r plotfit2, fig.height= 4, fig.width= 4}
# Compare the empirical distribution and the fitted values distribution
